...
window.nativeTreeTabs = {
  ...
  originalPinTab: null,
  originalAdoptTabGroup: null,
  originalReplaceGroupWithWindow: null,
  ...
  init: function() {
    ...
    if (this.tabPanels.length > 1) {
      if (defPanel && defPanel.count == 0) {
        //If default panel still exists, with zero tabs => remove it
        //(Window created from previously open tabs)
        this.tabPanels.splice(this.tabPanels.indexOf(defPanel), 1);
        removePanelFromMenu(defPanel.id);
      }
    }
  ...
  uninit: function() {
    ...
    gBrowser.adoptTabGroup = this.originalAdoptTabGroup;
    gBrowser.replaceGroupWithWindow = this.originalReplaceGroupWithWindow;
  ...
  checkMoveUnderOwnTree: function(aTab, prevPosition, newPosition, tabOriginalDepth) {
    let depthUpdate = false;
    let depthFix = 1;
    let possibleChildIndex = (prevPosition >= newPosition) ?
      prevPosition + 1 : prevPosition;
  ...
  checkForInsideCollapsedTreeMove: function(aTab, aTabTreeId, tabOriginalDepth, forceMultiselected, prevPosition, nextTab, groupMove = null) {
    if (isTab(nextTab) && nextTab.hasAttribute("hidden-child") &&
      ...
      if (groupMove == null) {
        ...
        nativeTreeTabs.moveTabsAfter(groupMove.tabs, newPosition);
        groupMove.tabs.forEach(function(gTab) {
  ...
  checkForPanelOverStep: function(aTab, prevPosition, tabOriginalDepth, group) {
    if (group == false && aTab.group && aTab.group.tabs.length == 1) {
      group = true;
    }
  ...
  tabMove: function(aTab, aEvent) {
      ...
      if (aTab.group != null) {
        if (prevPosition > newPosition) {
          if (aTab.group.tabs.indexOf(aTab) === aTab.group.tabs.length - 1) {
            //illegal 2
            if (this.checkForInsideCollapsedTreeMove(aTab, -1, 0, false, 0, getNextTab(aTab.group), aTab.group))
              return;
    ...
    if (this.checkForInsideCollapsedTreeMove(aTab, aTabTreeId, tabOriginalDepth, forceMultiselected, prevPosition, nextTab))
      return;
    ...
    if (telemetrySource == "drag" && (newPosition == gBrowser.tabs.length - 1 || (isTab(nextTab) && nextTab.hasAttribute("tabPanel-hidden")))) {
      setTreeDepth(aTab, '0');
  ...
  tabGroupDrag: function(target) {
    let tabgroup = target;
    let startDrag = function(aEvent) {
      tabgroup.removeEventListener("dragstart", startDrag, true);
      tabgroup.tabs.forEach(function(aTab) {
        aTab.setAttribute("tabGroupDrag", "true");
        setCustomTabValue(aTab, "draggedFromWindow", window.docShell.outerWindowID.toString());
      }, this);
      tabgroup.addEventListener("dragend", finisheDrag, true);
    };
    let finisheDrag = function(aEvent) {
      tabgroup.removeEventListener("dragend", finisheDrag, true);
      tabgroup.tabs.forEach(function(aTab) {
        aTab.removeAttribute("tabGroupDrag");
        setTimeout(() => {
          if (getCustomTabValue(aTab, "draggedFromWindow") == window.docShell.outerWindowID.toString()) {
            deleteCustomTabValue(aTab, "draggedFromWindow");
          }
        }, 1000);
      }, this);
    ...
    tabgroup.addEventListener("dragstart", startDrag, true);
  },
  ...
  tabSelected: function(aTab) {
    ...
    // function protectSelection() {
    if (aTab && (aTab.hasAttribute("nestTab") || getCustomTabValue(aTab, "nestTab") != "")) {
      if (nativeTreeTabs.selectedTab.hasAttribute("hidden-child") && nativeTreeTabs.selectedTab.getAttribute("hidden-child-rootID") == aTab.getAttribute("tree-id")) {
        aTab.setAttribute("skipUntwist", "");
      }
      let pSTab = nativeTreeTabs.selectedTab;
      while (pSTab.hasAttribute("nestTab")) {
        pSTab = nativeTreeTabs.previousSelectedTab.pop();
      }
      if (window.gBrowser.tabs.includes(pSTab) && pSTab.closing == false) {
        if (!pSTab.splitview) {
          gBrowser.selectedTab = pSTab;
  ...
  tabLeaveStrip: function(aTab, forceMultiselected = false) {
    ...
    while (nextTab) {
      ...
      if ((!multiSelected(nextTab) || forceMultiselected) && getCustomTabValue(nextTab, "draggedFromWindow") == "") {
        setTreeDepth(nextTab, newDepth);
      }
      nextTab = getNextTab(nextTab);
    }
  ...
  tabOpen: function(aTab) {
    ...
    this.observeTab(aTab, ["pending", "bursting", "open"], this.initTab);
  },
  ...
  attachTabListeners: function(aTab) {
    ...
    aTab.querySelector(".tab-close-button").addEventListener("click", this, true);
  },
  ...
  removeTabListeners: function() {
    gBrowser.tabs.forEach(function(aTab) {
      ...
      aTab.querySelector(".tab-close-button").removeEventListener("click", this, true);
  ...
  initTab: function(aTab) {
    ...
    if ((aTab.hasAttribute("tree-id") && dragged == "") || aTab.closing) {
      //already initialized
      return;
    }
    ...
    if (openerId) {
      ...
      // before tab opened => restore depth
      treeDepth = getTreeDepth(aTab);
      let possibleParent = getPreviousTab(aTab);
      let checked = 0;
  ...
  initTreeDepth: function(aTab) {
    ...
    let dragged = getCustomTabValue(aTab, "draggedFromWindow");
    setTimeout(() => {
      if (!aTab.hasAttribute("tree-id") || dragged != "") {
        this.initTab(aTab);
      }
    }, 100);
    return treeDepth;
  ...
  observeTab: function(target, attributeList, action) {
    let tabObserver = new MutationObserver(function(mutations, observer) {
      mutations.forEach(function(mutation) {
        if (mutation.type === "attributes") {
          let man = mutation.attributeName;
          if (attributeList.includes(man)) {
            observer.disconnect();
            action.call(nativeTreeTabs, mutation.target);
          }
        }
      });
    });
  ...
  defaultFunctionWrap: function() {
    ...
    gBrowser.removeTab = function(aTab, aOptions) {
            ...
            if (val === 0) {
              ...
              if (activeDepth > 0) {
                ...
                while (isTab(possibleSwitch)) {
                  ...
                  if (depth < activeDepth)
                    break;
                  if (depth == activeDepth && unloadedCheck(possibleSwitch) && tabVisible(possibleSwitch))
                    return possibleSwitch;
              ...
              while (isTab(possibleSwitch)) {
                if (possibleSwitch.hasAttribute("tabPanel-hidden"))
                  break;
                let depth = getTreeDepth(possibleSwitch);
                if (depth == 0 && unloadedCheck(possibleSwitch) && tabVisible(possibleSwitch))
                  return possibleSwitch;
                possibleSwitch = getPreviousTab(possibleSwitch)
              }
            } else if (val === 9) {
      ...
      try {
        if (aTab.hasAttribute("twisted-root") && aOptions && aOptions.metricsContext && aOptions.metricsContext && aOptions.metricsContext.telemetrySource &&
          gBrowser.TabMetrics.METRIC_SOURCE.MIDDLE_CLICK == aOptions.metricsContext.telemetrySource) {
          nativeTreeTabs.removeTree(aTab);
          return;
        }
    ...
    //Tab group transfer to another window
    this.originalAdoptTabGroup = gBrowser.adoptTabGroup;
    gBrowser.adoptTabGroup = function(group, {
      ...
      try {
        group.tabs.forEach(function(t) {
          skipNextMoveCheck(t);
          //get a bogus value to adopt the selected panel and not a previous one
          setCustomTabValue(t, "draggedFromWindow", "a");
        });
        nativeTreeTabs.originalAdoptTabGroup.apply(this, arguments);
        group.tabs.forEach(function(t) {
          removeSkipNextMoveCheck(t);
          deleteCustomTabValue(t, "draggedFromWindow");
        });
      } catch (error) {
        console.error(error);
        nativeTreeTabs.originalAdoptTabGroup.apply(this, arguments);
    ...
    this.originalReplaceGroupWithWindow = gBrowser.replaceGroupWithWindow;
    gBrowser.replaceGroupWithWindow = function(group, {
      ...
      try {
        group.tabs.forEach(function(t) {
          skipNextMoveCheck(t);
          setCustomTabValue(t, "draggedFromWindow", window.docShell.outerWindowID.toString());
        });
        nativeTreeTabs.originalReplaceGroupWithWindow.apply(this, arguments);
        group.tabs.forEach(function(t) {
          removeSkipNextMoveCheck(t);
          // deleteCustomTabValue(t, "draggedFromWindow");
        });
    ...
    groupProto.select = function() {
      this.collapsed = false;
      if (gBrowser.selectedTab.group == this) {
        return;
      }
      let index = 0;
      while (this.tabs[index] && (this.tabs[index].hasAttribute("nestTab") || getCustomTabValue(this.tabs[index], "nestTab") != "") && window.nativeTreeTabs._initialized) {
        index++;
      }
      if (this.tabs[index]) {
        ...
        window.gBrowser.moveTabToExistingGroup(newTab, this);
        setOpener(newTab, this.tabs[(this.tabs.length - 1)]);
        setTreeDepth(newTab, getTreeDepth(this.tabs[(this.tabs.length - 1)]) + 1);
        gBrowser.selectedTab = newTab;
      }
    };

    //Overwrite add, unsplit and tab reverse functions of the splitview object
    ...
    let splitviewsProto = customElements.get("tab-split-view-wrapper").prototype;
    let originalUnsplitTabs = splitviewsProto.unsplitTabs;
    splitviewsProto.unsplitTabs = function(trigger = null) {
      try {
        if (window.nativeTreeTabs._initialized == false) {
          originalUnsplitTabs.apply(this, arguments);
          return;
        }
        let saveFirstTabChildren;
        let firstTab;
        if (this.tabs != null && this.tabs.length > 1 && this.tabs[0] != null) {
          firstTab = this.tabs[0];
          saveFirstTabChildren = getFirstSplitViewTabChildren(this);
        }
        this.tabs.forEach(function(sTab) {
          skipNextMoveCheck(sTab);
        }, this);
        originalUnsplitTabs.apply(this, arguments);
        this.tabs.forEach(function(sTab) {
    ...
    let originalReverseTabs = splitviewsProto.reverseTabs;
    splitviewsProto.reverseTabs = function(trigger = null) {
      try {
        if (window.nativeTreeTabs._initialized == false) {
          originalReverseTabs.apply(this, arguments);
          return;
        }
        if (this.tabs.length > 1 && this.tabs[0].hasAttribute("twisted-root")) {
          nativeTreeTabs.toggleTwist(this.tabs[0]);
        }
        this.tabs.forEach(function(sTab) {
          skipNextMoveCheck(sTab);
        }, this);
    ...
    let originalAddTabs = splitviewsProto.addTabs;
    splitviewsProto.addTabs = function(tabs, {
      isSessionRestore = false,
      indexOfReplacedTab = -1
    } = {}) {
      try {
        if (window.nativeTreeTabs._initialized == false) {
          originalAddTabs.apply(this, arguments);
          return;
        }
        let splitDepth;
        let ogTabs = this.tabs.slice();
        let replacingTab = false;
        if (indexOfReplacedTab > -1 && this.tabs.length > 0 && tabs.length == 1 && !tabs[0].pinned) {
          replacingTab = true;
          movingTab = tabs[0];
          movingOldPosition = getPosition(movingTab);
          movingOldDepth = getTreeDepth(movingTab);
          insertBefore = this.tabs[0];
          ...
          isAncestor = (checkIfIsAncestor(insertBefore, movingTab)) ?
            true : false;
          skipNextMoveCheck(movingTab);
          makeSureOpenerIsSet(movingTab);
        }
        if (this.tabs.length > 0) {
          splitDepth = getTreeDepth(this.tabs[0]);
        }
        originalAddTabs.apply(this, arguments);
        this.tabs.forEach(function(sTab) {
          if (splitDepth != null && getTreeDepth(sTab) != splitDepth)
            setTreeDepth(sTab, splitDepth);
        }, this);
        if (replacingTab) {
          removeSkipNextMoveCheck(movingTab);
          if (isAncestor) {
      ...
      groupPopup.addEventListener("popupshowing", function(aEvent) {
        ...
        if (input && input.value === "") {
          if (TabContextMenu.contextTab) {
            let newTitle;
            if (TabContextMenu.contextTab.hasAttribute("nestTab")) {
              newTitle = TabContextMenu.contextTab.querySelector("[nestLabel]").textContent;
            } else {
              newTitle = TabContextMenu.contextTab.label;
            }
  ...
  moveTabBefore: function(tab, position, makeSureNoGroup = true) {
  ...
  moveTabAfter: function(tab, position, makeSureNoGroup = true) {
    //Move tab but not inside group
    if (position.splitview) {
      position = position.splitview;
    }
  ...
  tabPanelGetId: function(panel) {
    //receive id or panel object return id to string 
    let panelId;
    if (panel.id) {
  ...
  getTabPanelTabs: function(panel, reverse = false) {
    let panelId = this.tabPanelGetId(panel);
    if (panelId == -1) {
      return;
    }
    if (reverse)
  ...
  tabPanelClose: function(panel) {
    ...
    window.gBrowser.removeTabs(tabs, animate = false, suppressWarnAboutClosingWindow = false);
  },

  tabPanelReloadAll: function(panel) {
    let tabs = this.getTabPanelTabs(panel);
    gBrowser.reloadTabs(tabs);
  },

  tabPanelUnloadAllExpect: function(panel) {
    this.tabPanelShow(panel);
    let tabs = this.getTabPanelTabs(panel, reverse = true);
    let pinned = tabs.filter(t => t.pinned && !t.hasAttribute("discarded"));
    pinned.forEach((t) => {
      t.setAttribute("scriptDiscarded", "")
    });
    gBrowser.explicitUnloadTabs(tabs);
  },

  tabPanelUnloadAll: function(panel) {
    let tabs = this.getTabPanelTabs(panel);
    let pinned = tabs.filter(t => t.pinned && !t.hasAttribute("discarded"));
    pinned.forEach((t) => {
      t.setAttribute("scriptDiscarded", "")
  ...
  tabPanelBookmark: function(panel) {
    let tabs = this.getTabPanelTabs(panel);
    PlacesCommandHook.bookmarkTabs(tabs);
  },

  tabPanelCollapseTrees: function(panel, uncollapse = false) {
    let tabs = this.getTabPanelTabs(panel);
  ...
  tabPanelCollapseGroups: function(panel, uncollapse = false) {
    let groups = this.getTabPaneGroups(panel);
    let selectedRoot = getTreeRoot(gBrowser.selectedTab);
    groups.forEach((group) => {
      if (uncollapse)
        group.collapsed = false;

      else if (!group.tabs.includes(gBrowser.selectedTab)) {
  ...
  tabPanelShow: function(panel, changeSelectedTab = true) {
      ...
      if (aTab.hasAttribute("panel-id")) {
        if (aTab.getAttribute("panel-id") === panelId) {
          ...
          if (aTab.pinned && ((aTab.hasAttribute("pending") && aTab.getAttribute("pending") == "true") || aTab.hasAttribute("discarded"))) {
            if (aTab.hasAttribute("scriptDiscarded")) {
              aTab.removeAttribute("scriptDiscarded");
    ...
    if (changeSelectedTab) {
      ...
      if (pSTab && window.gBrowser.tabs.includes(pSTab) && (tabVisible(pSTab) || inNoCollapsedGroup(pSTab)) && unloadedCheck(pSTab) &&
        ...
        if (findTab == null) {
          findTab = window.gBrowser.tabContainer.allTabs.find(tab => tab.getAttribute("panel-id") === panelId && !tab.hasAttribute("nestTab"));
        }
        if (findTab == null) {
          let newTab = window.gBrowser.addTab(
            window.BROWSER_NEW_TAB_URL, {
...
setTreeDepth = function(aTab, depth) {
  ...
  if (isNaN(depth) || depth < 0) {
    console.error("Wrong value for tree depth");
    depth = 0;
  }
  let depthToString = depth.toString();
  if (aTab.splitViewId == null) {
    aTab.setAttribute("tree-depth", depthToString);
    setCustomTabValue(aTab, "tree-depth", depthToString);
  } else {
    aTab.tabs[0].setAttribute("tree-depth", depthToString);
    setCustomTabValue(aTab.tabs[0], "tree-depth", depthToString);
    aTab.tabs[1].setAttribute("tree-depth", depthToString);
    setCustomTabValue(aTab.tabs[1], "tree-depth", depthToString);
  }
  return depth;
}
...
menuItemRightClick = function(target, panel) {
  // let id = target.closest("menuitem").getAttribute("panel-id").toString();
  // let panel = nativeTreeTabs.tabPanels.find(x => x.id.toString() == id);
...
addNewPanelInMenu = function(panel, checkIt = false, position = -1) {
  ...
  let panelContext = document.getElementById('tab-panels-contextmenu');
  menuitem.addEventListener("auxclick", (aEvent) => {
    panelContext.contextElement = menuitem;
    panelContext.panel = panel;
    panelContext.openPopupAtScreen(aEvent.screenX, aEvent.screenY, false, aEvent)
  });
...
nestTabs = function(label) {
  ...
  let setNestAsOpener = function() {
    tabs.forEach(function(aTab) {
      setOpener(aTab, nestTab);
    });
  }

  window.nativeTreeTabs.observeTab(nestTab, ["tree-id"], setNestAsOpener);
...
addNestTabsInTabContextMenu = function() {
  ...
  function renameNest(label) {
    ...
    let nestLabel = nestTab.querySelector("[nestLabel]");
    if (nestLabel) {
      nestLabel.textContent = label;
    }
    nestTab.setAttribute("nestTab", label.toString());
    setCustomTabValue(nestTab, "nestTab", label.toString());
  }
...
addTabPanelButton = function(mainDiv) {
  ...
  //Create Tab Panel Context Menu
  let panelContext = document.createXULElement("menupopup");
  panelContext.setAttribute('id', 'tab-panels-contextmenu');
  elementsCreated.push(panelContext);
  mainPopupSet.appendChild(panelContext);


  function addContextItem(label, action, activeOnly = false) {
    let item = document.createXULElement('menuitem');
    item.setAttribute('label', label);
    if (activeOnly) {
      item.setAttribute('activeOnly', '');
    }
    item.addEventListener("click", (aEvent) => {
      action(aEvent)
    });
    panelContext.appendChild(item);
  }

  function addDoubleContextItem(label, label2, action, action2, activeOnly = false) {
    let hbox = document.createElement('hbox');
    hbox.setAttribute('class', "doubleMenuItem");

    let item1 = document.createXULElement('menuitem');
    let item2 = document.createXULElement('menuitem');
    item1.setAttribute('label', label);
    item1.addEventListener("click", (aEvent) => {
      action(aEvent)
    });
    item2.setAttribute('label', label2);
    item2.addEventListener("click", (aEvent) => {
      action2(aEvent)
    });
    if (activeOnly) {
      item1.setAttribute('activeOnly', '');
      item2.setAttribute('activeOnly', '');
    }
    hbox.appendChild(item1);
    hbox.appendChild(item2);
    panelContext.appendChild(hbox);

  }

  function addContextMenuSeperator(activeOnly = false) {
    let item = document.createXULElement('menuseparator');
    if (activeOnly) {
      item.setAttribute('activeOnly', '');
    }
    panelContext.appendChild(item);
  ...
  let contextMenuitemRename = addContextItem('Rename Panel', (aEvent) => {
    let contextElement = panelContext.contextElement;
    if (contextElement.tagName == "menuitem")
      menuItemRightClick(panelContext.contextElement, panelContext.panel);
    else {
  ...
  let contextMenuitemCollapseTrees = addContextItem('Expand trees', (aEvent) => {
    if (aEvent.ctrlKey)
      window.nativeTreeTabs.tabPanelCollapseGroups(panelContext.panel, true);
    window.nativeTreeTabs.tabPanelCollapseTrees(panelContext.panel, true);
  }, true);

  let contextMenuitemExpandTrees = addContextItem('Collapse trees', (aEvent) => {
    if (aEvent.ctrlKey)
      window.nativeTreeTabs.tabPanelCollapseGroups(panelContext.panel);
    window.nativeTreeTabs.tabPanelCollapseTrees(panelContext.panel);
  }, true);

  addContextMenuSeperator();


  let contextMenuitemClose = addContextItem('Close Panel Tabs', (aEvent) => {
    window.nativeTreeTabs.tabPanelClose(panelContext.panel);
  });


  panelContext.addEventListener("popupshowing", function(aEvent) {
    panelContext.childNodes.forEach(function(child) {
      let contextElement = panelContext.contextElement;
      if (contextElement.tagName == "menuitem" && child.hasAttribute("activeOnly"))
        child.style.display = "none";
      else {
        child.style.display = "";
      }
  ...
  tabPanelGroup.addEventListener("auxclick", (aEvent) => {
    ...
    aEvent.preventDefault();
    panelContext.contextElement = tabPanelGroup;
    panelContext.panel = window.nativeTreeTabs.selectedtPanel;
    panelContext.openPopupAtScreen(aEvent.screenX, aEvent.screenY, false, aEvent);
  });
